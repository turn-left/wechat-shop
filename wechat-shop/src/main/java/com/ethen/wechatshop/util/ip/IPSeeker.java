...
public class IPSeeker {
    ...
    public List<IPEntry> getIPEngries(String s) {
        ...
        for (int offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
            ...
            if (temp != -1) {
                ...
                if (loc.getCountry().indexOf(s) != -1 || loc.getIsp().indexOf(s) != -1) {
                    IPEntry entry = new IPEntry();
                    entry.country = loc.getCountry();
                    entry.isp = loc.getIsp();
                    // 得到起始IP
                    readIP(offset - 4, b4);
    ...
    private IPLocation getIPLocation(int offset) {
        ...
        if (b == isp_FOLLOWED) {
            ...
            // 读取地区标志
            loc.setIsp(readisp(mbb.position()));
        } else if (b == NO_isp) {
            loc.setCountry(readString(readInt3()));
            loc.setIsp(readisp(offset + 8));
        } else {
