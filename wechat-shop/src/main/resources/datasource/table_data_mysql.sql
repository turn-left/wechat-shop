...
(
  EC_ID          INT(10) primary key not null,-- 留言ID
...
(
  EN_ID          INT(10) primary key not null,
...
(
  EO_ID           INT(10) primary key not null,
  EO_USER_ID      VARCHAR(10) not null,-- 用户ID
  EO_USER_NAME    VARCHAR(20),-- 用户昵称
  EO_USER_ADDRESS VARCHAR(200),-- 用户地址
  EO_CREATE_TIME  DATE not null,-- 订单产生时间
  EO_COST         DECIMAL(10,2) not null,-- 总价钱
  EO_STATUS       INT(6) not null,-- 订单状态（待审核，发货......）
  EO_TYPE         INT(6) not null-- 付款类型（直接写死，不管）
);
-- 订单状态：
-- 1.待审核
-- 2.审核通过
...
(
  EOD_ID       INT(10) primary key not null,
  EO_ID        INT(10) not null,-- 订单ID
  EP_ID        INT(10) not null,-- 商品ID
  EOD_QUANTITY INT(6) not null,-- 购买数量
  EOD_COST     DECIMAL(10,2) not null-- 此商品的总价钱
);

-- 商品表
create table if not exists  EASYBUY_PRODUCT
(
  EP_ID          INT(10) primary key not null,
  EP_NAME        VARCHAR(20) not null,-- 商品名称
  EP_DESCRIPTION VARCHAR(100),-- 商品描述
  EP_PRICE       DECIMAL(10,2) not null,-- 商品单价
  EP_STOCK       INT(10) not null,-- 商品库存
  EPC_ID         INT(10),-- 商品分类父ID
  EPC_CHILD_ID   BIGINT(10),-- 商品分类子ID
  EP_FILE_NAME   VARCHAR(200)-- 商品图片名称
);

-- 商品分类表
create table if not exists  EASYBUY_PRODUCT_CATEGORY
(
  EPC_ID        BIGINT(10) primary key not null,-- 商品分类ID
  EPC_NAME      VARCHAR(20) not null,-- 商品分类名称
  EPC_PARENT_ID BIGINT(10) not null-- 商品分类父ID
);

-- 用户表
create table if not exists  EASYBUY_USER
(
  EU_USER_ID       VARCHAR(10) primary key not null,
  ...
  EU_STATUS        INT(6) not null-- 用户权限状态，1普通用户  2管理员
);
...
values (655, '刚订了台IPod，很是期待啊', str_to_date('22-12-2010', '%d-%m-%Y'), '货已发出，请注意收货时开箱检查货物是否有问题', str_to_date('22-12-2010', '%d-%m-%Y'), '小乖');
insert into EASYBUY_COMMENT (EC_ID, EC_CONTENT, EC_CREATE_TIME, EC_REPLY, EC_REPLY_TIME, EC_NICK_NAME)
values (680, '佳能D50现在可以多长时间发货呢', str_to_date('24-12-2010', '%d-%m-%Y'), '一般在订单确认后的第3天发货', str_to_date('24-02-2011', '%d-%m-%Y'), '无极');
commit;


insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (529, '最新酷睿笔记本', 'IBME系列全场促销中，最新酷睿双核处理器，保证CPU更高效的运转。', str_to_date('23-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (530, '团购无忧', '团购无忧', str_to_date('22-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (531, '会员特惠月开始了', '会员特惠月开始了', str_to_date('22-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (649, '加入会员，赢千万大礼包', '加入会员，赢千万大礼包', str_to_date('22-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (597, '迎双旦促销大酬宾', '迎双旦促销大酬宾', str_to_date('24-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (650, '新年不夜天，通宵也是开张了', '新年不夜天，通宵也是开张了', str_to_date('22-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (651, '积分兑换开始了', '积分兑换开始了', str_to_date('22-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (654, '配货通知', '配货通知', str_to_date('22-12-2010', '%d-%m-%Y'));
insert into EASYBUY_NEWS (EN_ID, EN_TITLE, EN_CONTENT, EN_CREATE_TIME)
values (653, '团购阿迪1折起', '团购阿迪1折起', str_to_date('22-12-2010', '%d-%m-%Y'));
commit;


insert into EASYBUY_ORDER (EO_ID, EO_USER_ID, EO_USER_NAME, EO_USER_ADDRESS, EO_CREATE_TIME, EO_COST, EO_STATUS, EO_TYPE)
values (747, 'sss', '小乖', '北京海淀区中关村大厦202', str_to_date('21-02-2011', '%d-%m-%Y'), 800, 1, 1);
commit;
...
insert into EASYBUY_USER (EU_USER_ID, EU_USER_NAME, EU_PASSWORD, EU_SEX, EU_BIRTHDAY, EU_IDENTITY_CODE, EU_EMAIL, EU_MOBILE, EU_ADDRESS, EU_STATUS)
values ('q', 'JQ', 'q', 'T', str_to_date('02-01-2010', '%d-%m-%Y'), null, null, null, null, 1);
insert into EASYBUY_USER (EU_USER_ID, EU_USER_NAME, EU_PASSWORD, EU_SEX, EU_BIRTHDAY, EU_IDENTITY_CODE, EU_EMAIL, EU_MOBILE, EU_ADDRESS, EU_STATUS)
